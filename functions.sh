...
echo "Document local value is $drupal_path"
  sed -i -- "s|@local_doc|${drupal_path}|g" ~/.drush/$site_url.drushrc.php


  # Change the Site URL for local
  sed -i -- "s|@local_url|$site_url|g" ~/.drush/$site_url.drushrc.php
  # Add the Entries for BI
  sed -i -- "s|@dev_url|$bi_site_url|g" ~/.drush/$site_url.drushrc.php
  sed -i -- "s|@dev_doc|$int_doc_root|g" ~/.drush/$site_url.drushrc.php
echo "Document Int value is $int_doc_root"
  sed -i -- "s|@dev_ssh|$bi_ssh_user|g" ~/.drush/$site_url.drushrc.php
  sed -i -- "s|@dev_host|$BI_HOST|g" ~/.drush/$site_url.drushrc.php
 # NOTE: sed create a backup file we don't need that.
 rm -f ~/.drush/$site_url.drushrc.php--
}
